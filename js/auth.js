// auth.js - VERSION 5.5 (DENGAN WHATSAPP INTEGRASI & LOGIN LOCKOUT SECURITY)
...
// PERUBAHAN V5.5: 
//   - Menambahkan sistem keamanan LOCKOUT setelah 3x percobaan login gagal
//   - Durasi lockout 1 menit, PERSISTEN meskipun halaman di-refresh
//   - Menggunakan localStorage untuk menyimpan data percobaan
//   - Countdown real-time pada tombol login
//   - Input dinonaktifkan selama masa lockout
// ============================================================================
...
const LOGIN_LOCKOUT_DURATION = 60; // 1 menit dalam detik
const MAX_LOGIN_ATTEMPTS = 3; // Maksimal 3 kali percobaan gagal
const STORAGE_KEY = 'login_attempts_data';

// Login attempt tracking - disimpan di localStorage (persistent)
let loginAttempts = {};

/**
 * Load saved login attempts dari localStorage
 */
function loadLoginAttempts() {
    try {
        ...
        if (saved) {
            ...
            console.log(`🔐 Loaded ${Object.keys(loginAttempts).length} login attempt records`);
        }
    } catch (e) {
...
/**
 * Save login attempts ke localStorage
 */
function saveLoginAttempts() {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(loginAttempts));
    } catch (e) {
...
/**
 * Get identifier untuk user (email)
 */
function getLoginIdentifier(email) {
    return email.toLowerCase().trim();
}

/**
 * Cek status lock untuk email tertentu
 */
function getLoginLockStatus(email) {
    ...
    // Jika ada lockUntil dan belum expired
...
/**
 * Handle login failure - increment attempts dan lock jika perlu
 */
function handleLoginFailure(email) {
    const identifier = getLoginIdentifier(email);
    const now = Math.floor(Date.now() / 1000);
    ...
    loginAttempts[identifier].attempts = (loginAttempts[identifier].attempts || 0) + 1;
    loginAttempts[identifier].lastAttempt = now;
    
    // Jika mencapai batas, lock
    if (loginAttempts[identifier].attempts >= MAX_LOGIN_ATTEMPTS) {
        loginAttempts[identifier].lockUntil = now + LOGIN_LOCKOUT_DURATION;
        console.log(`🔒 Login locked for ${email} for ${LOGIN_LOCKOUT_DURATION} seconds`);
    }
 ...
 */
function getRemainingTimeText(seconds) {
...
/**
 * Update UI lockout countdown
 */
function updateLockoutUI(email, remainingSeconds) {
    ...
    if (loginBtn) {
        ...
        loginBtn.textContent = `⏳ Tunggu ${getRemainingTimeText(remainingSeconds)}...`;
    ...
    if (errorMsg) {
        errorMsg.textContent = `🔒 Terlalu banyak percobaan gagal. Coba lagi dalam ${getRemainingTimeText(remainingSeconds)}.`;
    ...
    // Simpan interval untuk update countdown
    window._lockoutInterval = setInterval(() => {
        ...
        if (!status.isLocked) {
            ...
            if (errorMsg) {
                errorMsg.textContent = '✅ Silakan coba login kembali.';
            ...
            window.dispatchEvent(new CustomEvent('loginLockoutEnded', { 
                detail: { email: email } 
            }));
        } else {
            const remaining = status.remainingTime || 0;
            if (loginBtn) {
                loginBtn.textContent = `⏳ Tunggu ${getRemainingTimeText(remaining)}...`;
            }
            if (errorMsg) {
                errorMsg.textContent = `🔒 Terlalu banyak percobaan gagal. Coba lagi dalam ${getRemainingTimeText(remaining)}.`;
                errorMsg.style.display = 'block';
            }
        }
    }, 1000);
...
// ======================= FUNGSI LOGIN (DENGAN LOCKOUT SECURITY) =======================
...
function handleLogin(e) {
    ...
    if (lockStatus.isLocked) {
        const remaining = lockStatus.remainingTime || 0;
        showToast(`🔒 Akun terkunci. Coba lagi dalam ${getRemainingTimeText(remaining)}.`, "error");
        updateLockoutUI(email, remaining);
        return;
    }
    ...
    auth.signInWithEmailAndPassword(email, pass)
        .then((userCredential) => {
            ...
            return db.ref('users_auth/' + user.uid).once('value').then((snapshot) => {
                ...
                if (userData) {
                    ...
                    // Hilangkan error message jika ada
                    const errorMsg = document.getElementById('loginErrorMessage');
                    if (errorMsg) {
                        errorMsg.style.display = 'none';
                    }
        ...
        .catch((error) => {
            ...
            if (error.code === 'auth/user-not-found') {
                ...
                msg = "Password salah!";
                // Cek status lock setelah percobaan gagal
                const newStatus = getLoginLockStatus(email);
                if (newStatus.isLocked) {
                    const remaining = newStatus.remainingTime || 0;
                    msg = `🔒 Terlalu banyak percobaan gagal. Coba lagi dalam ${getRemainingTimeText(remaining)}.`;
                    updateLockoutUI(email, remaining);
                }
                ...
                // Firebase sudah memberikan lockout, kita tambahkan lockout lokal juga
        ...
        .finally(() => {
            // Jangan reset button jika masih dalam lockout
            const lockStatus = getLoginLockStatus(email);
            if (!lockStatus.isLocked) {
                btn.innerText = "MASUK";
                btn.disabled = false;
...
// ======================= INISIALISASI ====================
// Load saved login attempts saat halaman dimuat
loadLoginAttempts();

// Clean up lockout interval saat halaman ditutup
...
window.getLoginLockStatus = getLoginLockStatus;
window.resetLoginAttempts = resetLoginAttempts;
window.getRemainingTimeText = getRemainingTimeText;
window.LOGIN_LOCKOUT_DURATION = LOGIN_LOCKOUT_DURATION;
window.MAX_LOGIN_ATTEMPTS = MAX_LOGIN_ATTEMPTS;

console.log("✅ auth.js V5.5 loaded - REGISTRATION ONLY: SISWA & STAFF dengan validasi ID wajib & WhatsApp integration!");
console.log("🔐 Login Security: MAX_LOGIN_ATTEMPTS=" + MAX_LOGIN_ATTEMPTS + ", LOCKOUT_DURATION=" + LOGIN_LOCKOUT_DURATION + "s (PERSISTEN di localStorage)");
