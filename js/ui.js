    ...
    setTimeout(() => {
        if (currentUser && (currentUser.role === 'admin' || currentUser.role === 'guru' || currentUser.role === 'developer')) {
            ...
            if (typeof initAIAssistant === 'function') {
                ...
                console.warn("⚠️ initAIAssistant function not available yet, retrying in 2 seconds...");
                setTimeout(() => {
                    if (typeof initAIAssistant === 'function') {
                        initAIAssistant();
                    } else {
                        console.error("❌ initAIAssistant still not available after retry!");
                    }
                }, 2000);
            }
        } else if (currentUser) {
            console.log("🔒 AI Assistant not available for role:", currentUser.role);
        }
    }, 1500);
...
function renderUsersTable() {
    ...
    if (!dbData.users_auth || dbData.users_auth.length === 0) {
        tbody.innerHTML = `<td><td colspan="6" style="text-align:center; padding:20px; color:#888;">📭 Tidak ada pengguna ditemukan.${search ? '<br><small>Coba kata kunci lain</small>' : ''}<\/td><\/tr>`;
        return;
    }
    let data = dbData.users_auth.filter(u => u.nama && u.nama.toLowerCase().includes(search));
    if (data.length === 0) {
        tbody.innerHTML = `<td><td colspan="6" style="text-align:center; padding:20px; color:#888;">🔍 Tidak ada pengguna yang cocok dengan pencarian.${search ? '<br><small>Coba kata kunci lain</small>' : ''}<\/td><\/tr>`;
        return;
    }
    data.forEach(u => {
